...
Request.prototype._write = function (chunk, encoding, done) {
  var req = this.req;
  var write = function () {
    // We could technically ignore `encoding` since we haven't set
    // `decodeStrings` to true
    var consumed = req.write(chunk, encoding, done);
    if (!consumed) req.once('drain', write);
  };
  write();
};
